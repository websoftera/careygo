    ...
    function code39($x, $y, $code, $barWidth = 0.22, $barHeight = 6, $maxWidth = null)
    {
        ...
        if ($maxWidth !== null) {
            ...
            for ($i = 0, $len = strlen($code); $i < $len; $i++) {
                ...
                for ($j = 0; $j < 9; $j++) {
                    $totalWidth += ($pattern[$j] === 'w') ? $barWidth * 2.4 : $barWidth;
                }
                $totalWidth += $barWidth;
            }
            if ($totalWidth > 0 && $totalWidth > $maxWidth) {
                $barWidth *= $maxWidth / $totalWidth;
            }
        }
        $cursor = $x;
...
function generateReceiptPDF($shipment)
{
    ...
    if ($addr1_line2 !== '') {
        $pdf->SetXY($x, $y+=$lineH);
        $pdf->Cell(15, 5, "");
        receipt_value_cell($pdf, 77, 5, $addr1_line2);
    }

    if ($addr1_line3 !== '') {
        $pdf->SetXY($x, $y+=$lineH);
        $pdf->Cell(15, 5, "");
        receipt_value_cell($pdf, 77, 5, $addr1_line3);
    }
    ...
    $pdf->Cell(8, 5, "City:");
    $pdf->SetFont('Arial', 'B', $valueFont);
    receipt_value_cell($pdf, 28, 5, htmlspecialchars(receipt_upper($shipment['pickup_city'])));
    $pdf->SetFont('Arial', '', $labelFont);
    $pdf->Cell(10, 5, "State:");
    $pdf->SetFont('Arial', 'B', $valueFont);
    receipt_value_cell($pdf, 31, 5, htmlspecialchars(receipt_upper($shipment['pickup_state'])));
    $pdf->SetFont('Arial', '', $labelFont);
    $pdf->SetXY($x, $y+=$lineH);
    $pdf->SetFont('Arial', '', $labelFont);
    ...
    if ($addr2_line2 !== '') {
        $pdf->SetXY($x, $y+=$lineH);
        $pdf->Cell(15, 5, "");
        receipt_value_cell($pdf, 76, 5, $addr2_line2);
    }

    if ($addr2_line3 !== '') {
        $pdf->SetXY($x, $y+=$lineH);
        $pdf->Cell(15, 5, "");
        receipt_value_cell($pdf, 76, 5, $addr2_line3);
    }
    ...
    $pdf->Cell(8, 5, "City:");
    $pdf->SetFont('Arial', 'B', $valueFont);
    receipt_value_cell($pdf, 28, 5, htmlspecialchars(receipt_upper($shipment['delivery_city'])));
    $pdf->SetFont('Arial', '', $labelFont);
    $pdf->Cell(10, 5, "State:");
    $pdf->SetFont('Arial', 'B', $valueFont);
    receipt_value_cell($pdf, 31, 5, htmlspecialchars(receipt_upper($shipment['delivery_state'])));
    $pdf->SetFont('Arial', '', $labelFont);
    $pdf->SetXY($x, $y+=$lineH);
    $pdf->SetFont('Arial', '', $labelFont);
    $pdf->Cell(18, 5, "PIN Code:");
...
$pdf->currencyCell(50, 5, (float)$shipment['final_price']);
    ...
    // Barcode + AWB Number
    $pdf->code39(125, 147.5, receipt_barcode_payload($shipment), 0.17, 6, 70);
    $pdf->SetFont('Arial', '', 8);
    $pdf->SetXY(110, 155.2);
